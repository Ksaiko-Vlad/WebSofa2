...
export default function AdminMaterialList() {
  ...
  async function createMaterial(e: React.FormEvent) {
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) throw new Error(data?.message || 'Не удалось создать материал')
  ...
  async function saveEdit() {
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) throw new Error(data?.message || 'Не удалось обновить материал')
          ...
          <div className={s.formGrid}>
            ...
            <div className={s.field}>
              ...
              <input
                ...
                placeholder="0.00000"
              />
              <div className={s.hint}>Можно вводить строкой, бэк приведёт к Decimal.</div>
            </div>
              ...
              <tbody>
                {materials.map((m) => (
                  <tr key={m.id}>
                    ...
                    <td className={s.mono}>{Number(m.price_per_mm3).toString()}</td>
                    <td>
