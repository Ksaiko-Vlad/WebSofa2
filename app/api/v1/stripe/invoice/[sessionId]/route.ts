...
export async function GET(
  ...
  try {
    ...
    const wrapText = (t: string, size: number, maxWidth: number) => {
      ...
      for (const w of words) {
        ...
        if (textWidth(test, size) <= maxWidth) {
          ...
          // если одно слово слишком длинное — “жёстко” режем
    ...
    const deliveryHuman =
      meta.deliveryType === 'pickup' ? 'Самовывоз' : meta.deliveryType === 'home_delivery' ? 'Доставка' : ''

    const addressLine = meta.deliveryType === 'home_delivery'
    ...
    if (meta.note) {
      customerLines.push(`Комментарий: ${String(meta.note).slice(0, 180)}`)
    }

    // если строки длинные — переносим каждую по ширине блока
    const boxedLines: string[] = []
    for (const l of customerLines) {
      const wrapped = wrapText(l, 12, CONTENT_W - 24)
      boxedLines.push(...wrapped)
    }
    ...
    items.forEach((it: any, idx: number) => {
      ...
      const rowH = Math.max(baseRowH, rowPaddingY + nameLines.length * (12 + 4) + 8)
      ...
      const qtyText = `${qty}`
      page.drawText(qtyText, { x: xQty + 18, y: y - 18, size: 12, font, color: colors.text })

      // sum right aligned in its cell
